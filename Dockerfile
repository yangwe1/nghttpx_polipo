...
RUN ip route show | awk '/default/ {print $3}' > /etc/container_environment/ADDR && echo 9699 > /etc/container_environment/HTTPS_PORT &&\
...
RUN mkdir -p /etc/service/polipo && \
    ...
    printf "#!/bin/sh\n\nexec /usr/sbin/nghttpx -s -f'*,\$HTTPS_PORT' -b127.0.0.1,3128 /certs/\$KEY /certs/\$CERT\n" > /etc/service/nghttpx/run &&\
